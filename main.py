...
def loading_bar(add,  amount) -> str:
    """
    Function to create a loading bar
    :param add: the amount of the bar
    :param amount: the total amount of the bar
    :return: the loading bar
    """
    percentage = (int(add) / int(amount)) * 100
    if (percentage <= 33):
        color = '\033[31m\033[1m'
    elif (percentage * 100 <= 66):
...
class TikTok:
    def __init__(self):
        ...
        try:
            self.amount = int(input('> Number of Shares: '))
        except ValueError:
            self.close('Value error, number should be an integer')

        try:
            self.video_id = input('> TikTok Video URL: ').split('/')[5]
        except IndexError:
            self.close(
                'Bad TikTok URL format.\nFormat expected: https://www.tiktok.com/@username/vide'
    ...
    def bot(self):
        if self.added > 0:
            sys.stdout.write('\r')
            sys.stdout.write(loading_bar(self.added, self.amount))
            sys.stdout.flush()
